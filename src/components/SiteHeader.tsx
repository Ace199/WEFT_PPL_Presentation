...
export function SiteHeader({ current = "", boundaryId = "summary" }: { current?: string; boundaryId?: string }) {
  return <AutoHeader className={styles.header} boundaryId={boundaryId}>
    <Link prefetch={false} href="/" className={styles.wordmark} aria-label="WEFT / PPL">WEFT / PPL</Link>
    <LocalizedElement as="nav" label="章节导航">
      <Link prefetch={false} href="/" aria-current={current === "" ? "page" : undefined}><T text="00 / 项目概览" /></Link>
      {destinations.slice(0, 3).map(item => <Link prefetch={false} href={`/${item.slug}/`} key={item.slug} aria-current={current === item.slug ? "page" : undefined}>{item.index} / <T text={item.title} /></Link>)}
    </LocalizedElement>
