...
Locator: locator('[data-panel][data-active=true]').getByRole('link')
Expected: in viewport
Timeout: 5000ms
Error: element(s) not found

Call log:
  - Expect "toBeInViewport" locator('[data-panel][data-active=true]').getByRole('link') with timeout 5000ms
  - waiting for locator('[data-panel][data-active=true]').getByRole('link')
...
> 207 |     await expect(panel.getByRole("link")).toBeInViewport({ ratio: 1 });
      |                                           ^ Error: expect(locator).toBeInViewport() failed
  208 |     expect(
