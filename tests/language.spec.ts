...
test("language switch covers the homepage, scene and editor without changing view state", async ({page}) => {
  ...
  for (const width of [1440, 768, 390]) {
    ...
    for (const mode of ["organize", "transform", "extend"]) {
    ...
    await expect(page.locator("#views"), `after disclosure at ${width}`).toHaveAttribute("data-mode", "extend");
    ...
    await expect(page.locator("#views"), `after screenshot at ${width}`).toHaveAttribute("data-mode", "extend");
    await page.mouse.move(1, 1);
    await page.evaluate(() => window.scrollTo(0, 0));
    await expect(page.locator("#views"), `before language switch at ${width}`).toHaveAttribute("data-mode", "extend");
    await page.getByRole("button", {name: "切换为中文"}).click();
    await expect(page.locator("html")).toHaveAttribute("lang", "zh-CN");
    await expect(page.locator("#views")).toHaveAttribute("data-mode", "extend");
