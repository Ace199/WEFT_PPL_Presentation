...
- 02 页主标题改为“重新审视默认假设，由此形成新的设计思路。”，逗号后换行；英文对应为 “Revisit default assumptions. / Discover new design approaches.”。保留字号、字体、自然高度布局及其他副文案。
- 只改章节 Hero，不改首页 Transform 入口文案。入口为 `src/components/design/DesignPage.tsx`，导航测试标题断言同步更新。本次文案调整尚未提交或推送；下方回执属于此前版本。
