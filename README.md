...
导航与 Hero 合计填满首屏。Summary 顶线碰到导航底线后，导航随继续滚动逐渐上移，保留 8–10px 暖白边；反向滚动展开。鼠标移入、触屏点击或键盘进入仍可手动展开。状态信息按三条持续向上轮播，无暂停按钮，悬停 / 点击也不暂停；编辑和 reduced motion 下显示完整静态信息。

桌面 Master Canvas 在一屏内展示折叠状态，文字说明展开后向下延伸。页脚三圆首次可见后等待 1.5 秒播放，此后支持鼠标移入、键盘和触屏重播。后续会话请读 [开发交接文档](doc/WEFT_PPL_Development_Handoff_2026-09-17.md)，其中记录已确认的偏好、修正过的误读及已知验证限制。
